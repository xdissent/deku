...
function dom(world) {
  ...
  function mount(data) {
    ...
    var path = data.path;
    trigger('beforeMount', component, [ properties ]);
    var element = create(path, component, properties);
    root.appendChild(element);
    trigger('afterMount', component, [ element, properties ]);
  }
  ...
  function updateFrame() {
    ...
    Object.keys(changes).forEach(function(path){
      updateComponent(path, changes[path]);
    });
  }

  function updateComponent(path, properties) {
    ...
    var properties = merge(merge({}, previousProperties), properties);
  ...
  function diffNode(path, prev, next, el) {
    ...
    switch (next.type) {
      ...
      case 'component': return diffComponent(path + '.' + 0, prev, next, el);
    }
  }
  ...
  function diffComponent(path, prev, next, el) {
    if (next.component !== prev.component) {
      ...
      // replace properties, meaning update the component
      // after resetting the properties.
      updateComponent(path, next.props);
  ...
  function toNative(path, virtualElement) {
    switch (virtualElement.type) {
      ...
      // TODO: not sure if this is how it should really be done, adding `.0`.
      // but it works
      // should keep track of children potentially.
      case 'component': return toNativeComponent(path + '.' + 0, virtualElement);
    }
  }
  ...
  /**
   * TODO: potentially deprecate if it sounds good.
   */

  function trigger(hook, component, args) {
    if ('function' == typeof component[hook]) {
      component[hook].apply(component, args);
    }
    // this.lifecycle = name;
