...
function patch(entity, previousTree, nextTree, el, renderer){
  var context = {
    entity: entity,
  ...
  diffNode(previousTree.root, nextTree.root, context);
...
function diffChildren(previous, current, context){
  ...
  for (var i = 0; i < children.length; i++) {
    ...
    if (right == null) {
      removeComponents(left, childPath, context);
    ...
    diffNode(left, right, {
      ...
      path: childPath,
      previousTree: context.previousTree,
...
function replaceNode(current, next, context){
  ...
  removeComponents(current, context.path, context);
...
function removeComponents(node, path, context){
  ...
  if (node.children) {
    node.children.forEach(function(childNode, i){
      removeComponents(childNode, path + '.' + i, context);
    }, this);
  }
}
