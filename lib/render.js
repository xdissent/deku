...
function render (app, container, opts) {
  ...
  function removeNativeElement () {
    clearFrame()
    queueAction('remove', rootId, '0', currentNativeElement)
  ...
  function diffNode (path, entityId, prev, next, el) {
    ...
    if (prev.type !== next.type) return queueAction('replace', entityId, path, el, next)
  ...
  function diffComponent (path, entityId, prev, next, el) {
    if (next.component !== prev.component) {
      return queueAction('replace', entityId, path, el, next)
    } else {
  ...
  function diffElement (path, entityId, prev, next, el) {
    if (next.tagName !== prev.tagName) return queueAction('replace', entityId, path, el, next)
  ...
  function replaceElement (entityId, path, el, vnode) {
    ...
    queueAction('remove', entityId, path, el)
  ...
  /**
   * Queue add/remove action, for resolving animation properly.
   *
   * @param {String} type
   */
  ...
  function queueAction (type) {
    ...
    switch (type) {
      case 'remove': return removeElement.apply(null, args)
      case 'replace': return replaceElement.apply(null, args)
    }
  }
